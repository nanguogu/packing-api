...
- `DW2606-3070.cdr` 对应 `image-(15).png`，业务已确认其为一个整体灯牌。该文件由 CorelDRAW X8 Build 450 创建，只有一个通用“图层 1”、没有对象名称，却包含多个设计视图、4 个 group、140 条曲线和 20 个椭圆。这是整体判定的 golden case：系统应识别为一个刚性装箱实体，不得把圆内文字、鸟图案、重复视图或尺寸标注当成独立货物。

| Golden case | 预期分类 | 预期行为 |
| --- | --- | --- |
| `BW2606-3081.cdr` / `image-(6).png` | `separable` | 从生产轮廓识别独立字牌，排除尺寸线后分别排样 |
| `AW2606-3064单_生产文件.cdr` / 生产单 / `image-(22).png` | `separable` 或 `mixed` | 按独立字壳、公共底板和复合 Logo 形成实际刚性子组件，不能按“2 套”直接生成两个大矩形 |
| `DW2606-3070.cdr` / `DW2606-3070.xlsx` / `image-(15).png` | `mixed` | 圆形双面灯箱主体保持整体，内部图案和多视图不拆；生产单明确“安装脚：不锈钢、可拆：是”，因此安装脚/螺杆作为附件实体与主体同单装箱 |
...
2. **CDR 图层、group 和对象名称**：`PACK_OUTLINE`、材料层、切割层和稳定 group 可直接定义刚性组件；标注层、效果图层和打印图层不得生成装箱实体。历史 CDR 可能只有“图层 1”且没有对象名称，此时 group 数量不能直接等同于货物数量。
3. **生产单工艺**：灯箱、整板 UV、喷绘布、公共背板等提高“一体”置信度；普通背光字、独立字壳和逐字出线提高“可拆”置信度。生产单中的“套”只表示销售/生产数量，不等于一个刚性实体。
4. **PNG/效果图及多视图关系**：只做一致性检查。视觉上接触、叠放或共享颜色不能证明物理连接；同一页面中的正视图、背视图、尺寸图和效果图也不能重复计数为多个产品。

#### 装箱实体图

导入阶段先区分“设计视图”和“生产实体”，再构建连接图：候选物理轮廓为节点，确定的刚性连接为边。公共底板、同一实体材料轮廓、结构桥接和明确的刚性 group 可以建立边；电线、视觉接触和相邻关系不能建立刚性边。图中的每个连通分量成为一个 `packing_unit`。生产单数量决定实例数，但不会改变单个实例内部的刚性连接关系。

对整体产品，包装轮廓是所有不可拆刚性结构在包装姿态下的投影并集，而不是只取主视觉底板。`DW2606-3070` 的圆牌内部文字不拆；其生产单已明确安装脚可拆，所以安装脚不并入主体轮廓，而是作为独立附件与主体建立 `must_pack` 关系。若其他订单的安装臂不可拆，则必须计入主体箱宽和碰撞轮廓。
...
- `design_views`：被识别为正视图、背视图、尺寸图或效果图的页面区域，以及它们归属的同一产品实例；
...
- 默认业务目标是最低总成本。
- 首批 golden case 已确定为 `BW2606-3081.cdr`、`AW2606-3064单_生产单.xlsx`/对应 CDR，以及整体圆牌 `DW2606-3070.cdr`/`image-(15).png`。
- `DW2606-3070.xlsx` 已补齐：双面面光灯箱主体为整体，安装脚和螺杆可拆后同单装箱。
- 生产引擎采用 U-Nesting；不可用时允许进入人工复核/开发回退，但响应必须公开实际引擎名称。
- 使用 CorelDRAW Windows 转换节点，并保留人工上传 SVG 的备用路径。
- 当前所有成本比较统一使用现有香港至新加坡 Priority 公开价卡；响应保留请求目的地并明确 `pricing_lane_used=HK-SG`。
...
4. CDR 是否已有稳定图层/对象命名规范；转换节点实施时还需确定 CorelDRAW 版本和服务账号。
