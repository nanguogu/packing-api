...
| `DW2606-3070.cdr` / `image-(15).png` | `integrated` | 多个页面视图归并为同一产品；圆牌内部图案不拆，外伸安装臂计入整体包装外形 |
...
对整体产品，包装轮廓是所有刚性结构在包装姿态下的投影并集，而不是只取主视觉底板。例如 `DW2606-3070` 的圆牌内部文字不拆，但水平伸出的安装臂属于整体结构，必须计入箱宽和碰撞轮廓。
...
6. `DW2606-3070` 对应的生产单；CDR 已收到并确认，生产单可补充数量、材料、安装臂和包装工艺真值。
